...
local icon_description = {
	battery_ac     = { beautiful_name = 'delightful_battery_ac',     default_icon = function() return 'battery_plugged' end       },
	battery_full   = { beautiful_name = 'delightful_battery_full',   default_icon = function() return 'battery_full' end         },
	battery_medium_high = { beautiful_name = 'delightful_battery_medium_high', default_icon = function() return 'battery_two_thirds' end     },
	battery_medium = { beautiful_name = 'delightful_battery_medium', default_icon = function() return 'battery_two_thirds' end     },
	battery_medium_low = { beautiful_name = 'delightful_battery_medium_low', default_icon = function() return 'battery_two_thirds' end     },
	battery_low    = { beautiful_name = 'delightful_battery_low',    default_icon = function() return 'battery_low' end },
	not_found      = { beautiful_name = 'delightful_not_found',      default_icon = function() return 'battery_plugged' end },
	error          = { beautiful_name = 'delightful_error',          default_icon = function() return 'battery_plugged' end           },
}
...
function load(self, config)
	...
	if icon_files.battery_ac and icon_files.battery_full and icon_files.battery_medium_high and icon_files.battery_medium and icon_files.battery_medium_low and icon_files.battery_low and icon_files.not_found and icon_files.error then
...
function vicious_formatter(widget, data)
	...
	if icon then
		...
		elseif data[1] == '+' or data[1] == '↯' then
			icon_file = icon_files.battery_ac
		elseif data[2] >= 80 and data[2] <= 100 then
			icon_file = icon_files.battery_full
		elseif data[2] >= 60 and data[2] < 80   then
			icon_file = icon_files.battery_medium_high
		elseif data[2] >= 40 and data[2] < 60   then
			icon_file = icon_files.battery_medium
		elseif data[2] >= 20 and data[2] < 40   then
			icon_file = icon_files.battery_medium_low
		elseif data[2] >= 0  and data[2] < 20   then
